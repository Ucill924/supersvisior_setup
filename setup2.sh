...
apt update
apt install -y curl wget nano nvtop git supervisor screen build-essential pkg-config libssl-dev python3-dev
echo
...
echo "-----Writing PostgreSQL config (data_directory fixed to /data/postgresql)-----"
...
echo "-----Writing pg_hba.conf (allow remote, NOVITA PORT MAPPING)-----"
cat > /etc/postgresql/16/main/pg_hba.conf <<'PGHBA'
# TYPE  DATABASE        USER            ADDRESS                 METHOD
# Local connections
local   all             all                                     peer
# Allow ALL remote connections (NOVITA PORT MAPPING)
...
echo "-----Restarting postgres with new config-----"
supervisorctl restart dependencies:postgres
sleep 3
supervisorctl status dependencies:postgres
echo
